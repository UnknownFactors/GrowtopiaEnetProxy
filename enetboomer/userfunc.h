...
void GrowtopiaBot::userLoop() {
	...
	if (lastreq + 1000 < (duration_cast<milliseconds>(system_clock::now().time_since_epoch())).count()) { //execute only once in a second
		...
		for (int i = 0; i < 20; i++) { // shouldn't be more than 85 because of pps limit.
			...
			packet.m_state1 = x;
			packet.m_state2 = y;
			SendPacket2(PACKET_CALL_FUNCTION, (uint8_t*)&packet, sizeof(gameupdatepacket_t));
			SendPacket2(NET_MESSAGE_CLIENT_LOG_REQUEST, (uint8_t*)&packet, sizeof(gameupdatepacket_t));
			SendPacket2(NET_MESSAGE_CLIENT_LOG_RESPONSE, (uint8_t*)&packet, sizeof(gameupdatepacket_t));
			...
			cout << "Sent 1 stack of gameupdatepacket_t type NET_MESSAGE_CLIENT_LOG_REQUEST" << endl;
			cout << "Sent 1 stack of gameupdatepacket_t type NET_MESSAGE_CLIENT_LOG_RESPONSE" << endl;
			cout << "Sent 1 stack of gameupdatepacket_t type PACKET_CALL_FUNCTION" << endl;
		}
